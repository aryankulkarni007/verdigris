...
void ast_print_type(Type *t) {
  ...
  // Note: Add logic here to print array sizes or function signatures inline
}
...
void ast_print_stmt(Stmt *s, int indent, bool *last_mask, bool is_last) {
  ...
  case S_WHILE:
    printf("\n");
    ast_print_expr(s->as.while_loop.condition, indent + 1, last_mask, false);
    ast_print_stmt(s->as.while_loop.body, indent + 1, last_mask, true);
    break;
...
void ast_print_decl(Decl *d, int indent, bool *last_mask, bool is_last) {
  ...
  case D_PRIVATE:
    printf(" (pub: %s)\n", d->as.visibility.is_pub ? "true" : "false");
    ast_print_decl(d->as.visibility.inner, indent + 1, last_mask, true);
    break;
